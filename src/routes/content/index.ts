...
router.get(
  ...
  asyncHandler(async (req: ProtectedRequest, res: Response) => {
    ...
    try {
      ...
      const suggestion = await contentService.generateFormFillSuggestion(
        req.user._id,
        {
          category: category as string,
          seed: seed as string, // Optional seed for consistent results
        },
      );

      console.log('✅ Form fill suggestion generated successfully');
      ...
      // Return a smart fallback suggestion
      const fallbackSuggestion = {
        id: `fallback-${Date.now()}`,
        title: 'Content Marketing Best Practices for 2025',
        description:
          'Comprehensive guide to modern content marketing strategies that drive engagement and growth',
        category: 'Marketing',
        trending: false,
        reasoning:
          'Evergreen content marketing topic with consistent relevance',
        aiConfidence: 0.6,
      };
      ...
      new SuccessResponse('Form fill suggestion generated (fallback)', {
        ...
        formData: {
          ...
          tags: ['content-marketing', 'marketing', 'strategy', 'engagement'],
          priority: 'medium',
        },
        metadata: {
          autoFillReady: true,
...
// 📋 AI SUGGESTIONS FEED - For suggestions tab display
router.get(
  '/ai-suggest/feed',
  asyncHandler(async (req: ProtectedRequest, res: Response) => {
    ...
    try {
      // Generate multiple trending content suggestions for the suggestions tab
      const suggestions = await contentService.generateSuggestionsFeed(
        req.user._id,
        {
      ...
      console.log('✅ AI Suggestions Feed generated successfully');

      // Each suggestion includes embedded formData for immediate use
      new SuccessResponse('AI suggestions feed retrieved successfully', {
        suggestions: suggestions.map((suggestion: any) => ({
          // Display data for the suggestions tab
          id: suggestion.id,
          title: suggestion.title,
          description: suggestion.description,
          category: suggestion.category,
          ...
          timeToCreate: suggestion.timeToCreate,
          reasoning: suggestion.reasoning,
          aiConfidence: suggestion.aiConfidence,
          createdAt: suggestion.createdAt,

          // Embedded form data for "Create Content" button
          formData: {
            title: suggestion.title,
            description: suggestion.description,
            type: suggestion.contentType,
        ...
        metadata: {
          ...
          feedType: 'trending_topics',
          source: 'ai_dashboard_feed',
        },
      }).send(res);
    } catch (error) {
      console.error('❌ Error in AI suggestions feed:', error);

      // Return fallback suggestions for the feed
      const fallbackSuggestions = [
        {
          id: 'fallback-1',
          title: 'Top 6 AI Tools for Content Creators in 2025',
          description:
            'Explore the most effective AI tools that are revolutionizing content creation',
          contentType: 'article',
          platforms: ['linkedin', 'medium', 'twitter'],
          tags: ['ai-tools', 'content-creation', 'productivity', 'technology'],
          priority: 'high',
          trending: true,
          category: 'Technology',
          ...
          reasoning:
            'AI tools are highly trending and relevant for content creators',
        },
        {
          id: 'fallback-2',
          title: 'Social Media Trends to Watch This Week',
          description:
            'Latest social media trends and how to leverage them for better engagement',
          contentType: 'social',
          platforms: ['instagram', 'tiktok', 'twitter'],
          tags: ['social-media', 'trends', 'engagement', 'marketing'],
          priority: 'high',
          trending: true,
          ...
          reasoning:
            'Social media trends are time-sensitive and high-engagement',
        },
        {
          id: 'fallback-3',
          title: 'Productivity Hacks for Remote Teams',
          description:
            'Proven strategies to boost productivity in remote work environments',
          contentType: 'video',
          platforms: ['youtube', 'linkedin', 'tiktok'],
          tags: ['productivity', 'remote-work', 'team-management', 'business'],
          priority: 'medium',
          trending: false,
          category: 'Business',
          ...
          reasoning: 'Remote work content remains consistently popular',
        },
      ];
